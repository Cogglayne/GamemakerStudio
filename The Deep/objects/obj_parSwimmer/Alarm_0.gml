...
if (obj_microtransactions.microtransactions[6] == false || room == TutorialAngler || room == TutorialMine || room == TutorialMonster || room == TutorialProbe || room == TutorialMonsterEvolvedOne || room == TutorialMonsterEvolvedTwo || room == TutorialMovement) {
    timer++;
    // text to speek for timer if the option is activated
    if (obj_options.options[7] && singleplayer) {
        switch (timer) {
            case 10:
